...
import art.arcane.volmlib.util.hunk.storage.AtomicDoubleHunk;
import art.arcane.volmlib.util.hunk.storage.AtomicHunk;
import art.arcane.volmlib.util.hunk.storage.AtomicIntegerHunk;
import art.arcane.volmlib.util.hunk.storage.AtomicLongHunk;
import art.arcane.volmlib.util.hunk.storage.MappedHunk;
import art.arcane.volmlib.util.hunk.storage.MappedSyncHunk;
import art.arcane.volmlib.util.hunk.storage.SynchronizedArrayHunk;
import art.arcane.volmlib.util.hunk.view.DriftHunkView;
import art.arcane.volmlib.util.hunk.view.FringedHunkView;
import art.arcane.volmlib.util.hunk.view.FunctionalHunkView;
import art.arcane.volmlib.util.hunk.view.HunkView;
import art.arcane.volmlib.util.hunk.view.InvertedHunkView;
import art.arcane.volmlib.util.hunk.view.ListeningHunk;
import art.arcane.volmlib.util.hunk.view.ReadOnlyHunk;
import art.arcane.volmlib.util.hunk.view.SynchronizedHunkView;
import art.arcane.volmlib.util.hunk.view.WriteTrackHunk;
...
public interface Hunk<T> extends HunkLike<T> {
    ...
    static <A, B> Hunk<B> convertedReadView(Hunk<A> src, Function<A, B> reader) {
        return new FunctionalHunkView<>(src, reader, null);
    }

    static <A, B> Hunk<B> convertedWriteView(Hunk<A> src, Function<B, A> writer) {
        return new FunctionalHunkView<>(src, null, writer);
    }
    ...
    static <T> Hunk<T> newMappedHunkSynced(int w, int h, int d) {
        return new MappedSyncHunk<>(w, h, d);
    }
    ...
    static <A, B> void computeDual2D(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Hunk<A> a, Hunk<B> b, Consumer5<Integer, Integer, Integer, Hunk<A>, Hunk<B>> v) {
    ...
    static BlockPosition rotatedBounding(int w, int h, int d, double x, double y, double z) {
        int[] bounds = HunkCoreSupport.rotatedBoundsSize(w, h, d, x, y, z);
        return new BlockPosition(bounds[0], bounds[1], bounds[2]);
    }
    ...
    default int getNonNullEntries(Function<Integer, ? extends BurstExecutorSupport> executor) {
        AtomicInteger count = new AtomicInteger();
        iterate(executor, (x, y, z, value) -> count.getAndAdd(1));
        return count.get();
    }
     ...
     * @return this
     */
    default Hunk<T> iterateSurfaces2D(Function<Integer, ? extends BurstExecutorSupport> executor, Predicate<T> p, Consumer8<Integer, Integer, Integer, Integer, Integer, Integer, Integer, Hunk<T>> c) {
        return iterateSurfaces2D(getIdeal2DParallelism(), executor, p, c);
    }
     ...
     * @return this
     */
    default Hunk<T> iterateSurfaces2D(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Predicate<T> p, Consumer8<Integer, Integer, Integer, Integer, Integer, Integer, Integer, Hunk<T>> c) {
        iterate2DTop(parallelism, executor, (ax, az, hox, hoz, h) ->
        {
            int last = -1;
            int in = getHeight() - 1;
     ...
     * @return this
     */
    default Hunk<T> iterate2DTop(Function<Integer, ? extends BurstExecutorSupport> executor, Consumer5<Integer, Integer, Integer, Integer, Hunk<T>> c) {
        return iterate2DTop(getIdeal2DParallelism(), executor, c);
    }
     ...
     * @return this
     */
    default Hunk<T> iterate2DTop(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Consumer5<Integer, Integer, Integer, Integer, Hunk<T>> c) {
        compute2D(parallelism, executor, (x, y, z, h) ->
        {
    ...
    default Hunk<T> iterate(Function<Integer, ? extends BurstExecutorSupport> executor, Predicate<T> p, Consumer3<Integer, Integer, Integer> c) {
        return iterate(getIdeal3DParallelism(), executor, p, c);
    }

    default Hunk<T> iterate(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Predicate<T> p, Consumer3<Integer, Integer, Integer> c) {
        iterate(parallelism, executor, (x, y, z, t) ->
        {
            if (p.test(t)) {
                c.accept(x, y, z);
            }
    ...
    default Hunk<T> iterate(Function<Integer, ? extends BurstExecutorSupport> executor, Predicate<T> p, Consumer4<Integer, Integer, Integer, T> c) {
        return iterate(getIdeal3DParallelism(), executor, p, c);
    }

    default Hunk<T> iterate(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Predicate<T> p, Consumer4<Integer, Integer, Integer, T> c) {
        iterate(parallelism, executor, (x, y, z, t) ->
        {
            if (p.test(t)) {
                c.accept(x, y, z, t);
    ...
    default Hunk<T> iterate(Function<Integer, ? extends BurstExecutorSupport> executor, Consumer3<Integer, Integer, Integer> c) {
        return iterate(getIdeal3DParallelism(), executor, c);
    }
    ...
    default Hunk<T> iterate(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Consumer3<Integer, Integer, Integer> c) {
        compute3D(parallelism, executor, (x, y, z, h) ->
        {
            for (int i = 0; i < h.getWidth(); i++) {
    ...
    default Hunk<T> iterate(Function<Integer, ? extends BurstExecutorSupport> executor, Consumer4<Integer, Integer, Integer, T> c) {
        return iterate(getIdeal3DParallelism(), executor, c);
    }

    default Hunk<T> iterate(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Consumer4<Integer, Integer, Integer, T> c) {
        compute3D(parallelism, executor, (x, y, z, h) ->
        {
            for (int i = 0; i < h.getWidth(); i++) {
                for (int j = 0; j < h.getHeight(); j++) {
    ...
    default Hunk<T> compute2D(Function<Integer, ? extends BurstExecutorSupport> executor, Consumer4<Integer, Integer, Integer, Hunk<T>> v) {
        return compute2D(getIdeal2DParallelism(), executor, v);
    }

    default Hunk<T> compute2D(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Consumer4<Integer, Integer, Integer, Hunk<T>> v) {
    ...
    default Hunk<T> compute2DYRange(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, int ymin, int ymax, Consumer4<Integer, Integer, Integer, Hunk<T>> v) {
        if (get2DDimension(parallelism) == 1) {
            v.accept(0, 0, 0, this);
            return this;
        }
    ...
    default Hunk<T> compute3D(Function<Integer, ? extends BurstExecutorSupport> executor, Consumer4<Integer, Integer, Integer, Hunk<T>> v) {
        return compute3D(getIdeal3DParallelism(), executor, v);
    }

    default Hunk<T> compute3D(int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor, Consumer4<Integer, Integer, Integer, Hunk<T>> v) {
    ...
    default void set(int x, int y, int z, T t) {
        if (!contains(x, y, z)) {
            ...
            VolmLog.fine("Hunk", "OUT OF BOUNDS " + x + " " + y + " " + z + " in bounds " + getWidth() + " " + getHeight() + " " + getDepth(), null);
            return;
        }
