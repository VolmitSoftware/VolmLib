...
public final class ConfigRewriteReporter {
    private static final int MAX_KEYS_PER_CATEGORY = 8;
    private static final String MISSING = "<missing>";
    private static final String REMOVED = "<removed>";

    private ConfigRewriteReporter() {
    }

    public static void reportRewrite(ConfigIo io, File file, String source, String beforeRaw, String afterRaw) {
        ...
        String path = relativize(io, file);
        String sourceTag = source == null || source.isBlank() ? "config" : source;

        if (changes.isEmpty()) {
            io.info("Canonicalized " + sourceTag + " [" + path + "] (format/order only).");
            return;
        }
        ...
        io.warn("Canonicalized " + sourceTag + " [" + path + "] with schema changes (removed=" + removed + ", added=" + added + ", changed=" + changed + ").");
        if (!removedKeys.isEmpty()) {
            io.warn(" - removed keys: " + summarizeKeys(removedKeys));
        }
        if (!addedKeys.isEmpty()) {
            io.info(" - added keys: " + summarizeKeys(addedKeys));
        }
        if (!changedKeys.isEmpty()) {
            io.info(" - changed keys: " + summarizeKeys(changedKeys));
        }
    }

    public static void reportFallbackRewrite(ConfigIo io, File file, String source, String reason) {
        String path = relativize(io, file);
        String sourceTag = source == null || source.isBlank() ? "config" : source;
        String reasonText = reason == null || reason.isBlank() ? "invalid/unsupported content" : reason;
        io.warn("Rewrote " + sourceTag + " [" + path + "] using fallback defaults (" + reasonText + ").");
    }
    ...
    private static String relativize(ConfigIo io, File file) {
        if (file == null) {
            return "<unknown>";
        }
