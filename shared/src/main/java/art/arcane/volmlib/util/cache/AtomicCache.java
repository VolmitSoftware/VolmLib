...
public class AtomicCache<T> {
    ...
    private static final Logger LOG = Logger.getLogger(AtomicCache.class.getName());
    ...
    public T aquireNastyPrint(NastySupplier<T> t) {
        return aquire(() -> {
            try {
                ...
                LOG.log(Level.SEVERE, "Atomic cache supplier failed.", e);
                return null;
            }
        });
    }
    ...
    public T aquire(Supplier<T> t) {
        ...
        synchronized (initLock) {
            ...
            try {
                ...
                if (REPORTED_FAILURES.size() < 512 && REPORTED_FAILURES.add(failureKey)) {
                    LOG.log(Level.WARNING, "Atomic cache supplier failed: " + failureKey, e);
                }
            }
