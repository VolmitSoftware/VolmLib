...
import art.arcane.volmlib.util.stream.arithmetic.AddingStream;
import art.arcane.volmlib.util.stream.arithmetic.ClampedStream;
import art.arcane.volmlib.util.stream.arithmetic.CoordinateBitShiftLeftStream;
import art.arcane.volmlib.util.stream.arithmetic.CoordinateBitShiftRightStream;
import art.arcane.volmlib.util.stream.arithmetic.DividingStream;
import art.arcane.volmlib.util.stream.arithmetic.FittedStream;
import art.arcane.volmlib.util.stream.arithmetic.MaxingStream;
import art.arcane.volmlib.util.stream.arithmetic.MinningStream;
import art.arcane.volmlib.util.stream.arithmetic.ModuloStream;
import art.arcane.volmlib.util.stream.arithmetic.MultiplyingStream;
import art.arcane.volmlib.util.stream.arithmetic.OffsetStream;
import art.arcane.volmlib.util.stream.arithmetic.RadialStream;
import art.arcane.volmlib.util.stream.arithmetic.RoundingDoubleStream;
import art.arcane.volmlib.util.stream.arithmetic.SlopeStream;
import art.arcane.volmlib.util.stream.arithmetic.SubtractingStream;
import art.arcane.volmlib.util.stream.arithmetic.ZoomStream;
import art.arcane.volmlib.util.stream.convert.AwareConversionStream2D;
import art.arcane.volmlib.util.stream.convert.AwareConversionStream3D;
import art.arcane.volmlib.util.stream.convert.CachedConversionStream;
import art.arcane.volmlib.util.stream.convert.ConversionStream;
import art.arcane.volmlib.util.stream.convert.ForceDoubleStream;
import art.arcane.volmlib.util.stream.convert.RoundingStream;
import art.arcane.volmlib.util.stream.convert.SelectionStream;
...
public interface ProceduralStream<T> extends ProceduralLayer, Interpolated<T> {
    ...
    default <V extends Rarity> ProceduralStream<V> selectRarity(List<V> types) {
        return Rarity.stream(this.forceDouble(), types);
    }

    default <V> ProceduralStream<Rarity> selectRarity(List<V> types, Function<V, Rarity> loader) {
        List<Rarity> r = new ArrayList<>();
        for (V f : types) {
    ...
    default <V> int countPossibilities(List<V> types, Function<V, Rarity> loader) {
        KList<V> rarityTypes = new KList<>();
        int totalRarity = 0;
        for (V i : types) {
            totalRarity += Rarity.get(loader.apply(i));
        }

        for (V i : types) {
            rarityTypes.addMultiple(i, totalRarity / Rarity.get(loader.apply(i)));
        }
    ...
    default Hunk<T> fastFill2DParallel(int x, int z, Function<Integer, ? extends BurstExecutorSupport> executor) {
        Hunk<T> hunk = Hunk.newAtomicHunk(16, 16, 1);
        BurstExecutorSupport e = executor.apply(256);
        int i, j;
    ...
    default void fastFill2DParallel(Hunk<T> hunk, BurstExecutorSupport e, int x, int z) {
        int i, j;
    ...
    default void fill(Hunk<T> h, double x, double y, double z, int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor) {
        h.compute3D(parallelism, executor, (xx, yy, zz, hh) -> hh.iterate(executor, (xv, yv, zv) -> hh.set(xv, yv, zv, get(xx + xv + x, yy + yv + y, zz + zv + z))));
    }

    default <V> void fill2D(Hunk<V> h, double x, double z, V v, int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor) {
        h.compute2D(parallelism, executor, (xx, __, zz, hh) ->
        {
    ...
    default <V> void fill2D(Hunk<V> h, double x, double z, ProceduralStream<V> v, int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor) {
        h.compute2D(parallelism, executor, (xx, yy, zz, hh) ->
        {
            for (int i = 0; i < hh.getWidth(); i++) {
                for (int k = 0; k < hh.getDepth(); k++) {
                    double n = getDouble(i + x + xx, k + z + zz);
    ...
    default <V> void fill2DYLocked(Hunk<V> h, double x, double z, V v, int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor) {
        h.compute2D(parallelism, executor, (xx, yy, zz, hh) ->
        {
            for (int i = 0; i < hh.getWidth(); i++) {
                for (int k = 0; k < hh.getDepth(); k++) {
    ...
    default <V> void fill2DYLocked(Hunk<V> h, double x, double z, ProceduralStream<V> v, int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor) {
        h.compute2D(parallelism, executor, (xx, yy, zz, hh) ->
        {
            for (int i = 0; i < hh.getWidth(); i++) {
                for (int k = 0; k < hh.getDepth(); k++) {
    ...
    default <V> void fill3D(Hunk<V> h, double x, int y, double z, V v, int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor) {
        h.compute3D(parallelism, executor, (xx, yy, zz, hh) -> hh.iterate(executor, (xv, yv, zv) ->
        {
    ...
    default <V> void fill3D(Hunk<V> h, double x, int y, double z, ProceduralStream<V> v, int parallelism, Function<Integer, ? extends BurstExecutorSupport> executor) {
        h.compute3D(parallelism, executor, (xx, yy, zz, hh) -> hh.iterate(executor, (xv, yv, zv) ->
        {
            if (getDouble(xx + xv + x, yy + yv + y, zz + zv + z) > 0.5) {
    ...
    default void fill(Hunk<T> h, double x, double y, double z, Function<Integer, ? extends BurstExecutorSupport> executor) {
        fill(h, x, y, z, 4, executor);
    }

    default <V> void fill2D(Hunk<V> h, double x, double z, V v, Function<Integer, ? extends BurstExecutorSupport> executor) {
        fill2D(h, x, z, v, 4, executor);
    }

    default <V> void fill2D(Hunk<V> h, double x, double z, ProceduralStream<V> v, Function<Integer, ? extends BurstExecutorSupport> executor) {
        fill2D(h, x, z, v, 4, executor);
    }

    default <V> void fill2DYLocked(Hunk<V> h, double x, double z, V v, Function<Integer, ? extends BurstExecutorSupport> executor) {
        fill2DYLocked(h, x, z, v, 4, executor);
    }

    default <V> void fill2DYLocked(Hunk<V> h, double x, double z, ProceduralStream<V> v, Function<Integer, ? extends BurstExecutorSupport> executor) {
        fill2DYLocked(h, x, z, v, 4, executor);
    }

    default <V> void fill3D(Hunk<V> h, double x, int y, double z, V v, Function<Integer, ? extends BurstExecutorSupport> executor) {
        fill3D(h, x, y, z, v, 4, executor);
    }

    default <V> void fill3D(Hunk<V> h, double x, int y, double z, ProceduralStream<V> v, Function<Integer, ? extends BurstExecutorSupport> executor) {
        fill3D(h, x, y, z, v, 4, executor);
    }
