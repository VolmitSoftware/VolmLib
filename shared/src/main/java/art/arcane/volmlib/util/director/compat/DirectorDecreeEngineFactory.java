...
public final class DirectorDecreeEngineFactory {
    private DirectorDecreeEngineFactory() {
    }
    ...
    private static DirectorRuntimeNode buildTree(Object instance, DirectorRuntimeNode parent) {
        ...
        DirectorNodeDescriptor groupDescriptor = DirectorDecreeCompatibility.fromType(type)
                .orElseThrow(() -> new IllegalStateException("Command type is missing @Decree annotation: " + type.getName()));
        ...
        for (Field field : type.getDeclaredFields()) {
            ...
            if (!field.getType().isAnnotationPresent(Decree.class)) {
                continue;
            }
    ...
    private static DirectorRuntimeNode buildMethodNode(Object instance, Method method, DirectorRuntimeNode parent) {
        DirectorNodeDescriptor descriptor = DirectorDecreeCompatibility.fromMethod(method)
                .orElseThrow(() -> new IllegalStateException("Method is missing @Decree annotation: " + method.getName()));
    ...
    private static Object readOrCreateChild(Object parent, Field field) {
        try {
            ...
            throw new IllegalStateException(
                    "Failed to initialize @Decree field " + field.getDeclaringClass().getName() + "." + field.getName(), e);
        }
    }
}
