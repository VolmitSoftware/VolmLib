...
public class KList<T> extends ArrayList<T> implements List<T> {
    ...
    public T popRandom() {
        ...
        return remove(ThreadLocalRandom.current().nextInt(last() + 1));
    }

    public T popRandom(Random rng) {
        if (isEmpty()) {
            return null;
        }
        ...
        return remove(rng.nextInt(last() + 1));
    }

    public KList<T> sub(int f, int t) {
        KList<T> g = new KList<>();

        for (int i = f; i < Math.min(size(), t); i++) {
            g.add(get(i));
        }
    ...
    public T getRandom() {
        ...
        return get(ThreadLocalRandom.current().nextInt(last() + 1));
    }

    public KList<T> popRandom(Random rng, int c) {
        KList<T> m = new KList<>();
