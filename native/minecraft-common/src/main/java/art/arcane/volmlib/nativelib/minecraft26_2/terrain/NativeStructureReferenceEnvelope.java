...
public final class NativeStructureReferenceEnvelope {
    private static final System.Logger LOGGER = System.getLogger(NativeStructureReferenceEnvelope.class.getName());
    ...
    public static StructureStart wrap(StructureStart generated, Structure source, int references,
                                      StructureTerrainSettings terrain, String structureKey) {
    ...
    public static BoundingBox referenceBounds(StructureStart start, Structure source,
                                              StructureTerrainSettings terrain,
                                              String structureKey) {
        StructureTerrainMode mode = terrain == null
                ? StructureTerrainMode.PRESERVE : terrain.resolvedMode();
        boolean usesEnvelope = mode == StructureTerrainMode.BORE
                || mode == StructureTerrainMode.FORCE_CARVE
                || mode == StructureTerrainMode.VACUUM
                || mode == StructureTerrainMode.FLATTEN
                || mode == StructureTerrainMode.ENCASE;
        int horizontalPadding = mode == StructureTerrainMode.VACUUM
        ...
        try {
            ...
            adjustedEnvelope = mode == StructureTerrainMode.SOURCE
    ...
    private static void warnClippedTerrain(
        ...
        if (WARNED_CLIPPED_TERRAIN.add(key)) {
            LOGGER.log(System.Logger.Level.WARNING, "Clipping optional terrain envelope for native structure '"
    ...
    private static void warnSkippedContent(
        ...
        if (WARNED_SKIPPED_CONTENT.add(key)) {
            LOGGER.log(System.Logger.Level.WARNING, "Skipping native structure '" + key + "' at "
                    + start.getChunkPos().x() + "," + start.getChunkPos().z()
