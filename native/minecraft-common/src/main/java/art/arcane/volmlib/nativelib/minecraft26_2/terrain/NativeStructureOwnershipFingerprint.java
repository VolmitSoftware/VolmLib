...
import art.arcane.volmlib.nativelib.terrain.structure.StructureOwnershipView;
...
public final class NativeStructureOwnershipFingerprint {
    ...
    public static StructureFingerprint capture(String structureKey, StructureStart start,
        ...
        return new StructureFingerprint(
                structureKey, origin.x(), origin.z(),
                content.minX(), content.minY(), content.minZ(),
                content.maxX(), content.maxY(), content.maxZ(),
                locatorY(resolvedStart), references.minX() >> 4, references.maxX() >> 4,
                references.minZ() >> 4, references.maxZ() >> 4,
                fingerprint(structureKey, resolvedStart));
    }

    public static boolean matches(StructureOwnershipView record, StructureStart start) {
