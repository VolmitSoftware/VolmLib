...
public final class VanillaStructureBiomes {
    private VanillaStructureBiomes() {
    }

    public static Set<String> possibleBiomeKeys(BiomeSource source) {
        if (source == null) {
            throw new IllegalStateException("Minecraft chunk generator has no biome source");
        }
        Set<String> keys = new LinkedHashSet<>();
        Set<Holder<Biome>> possibleBiomes = source instanceof StructureBiomeSource customBiomeSource
    ...
    public static Set<String> structureBiomeKeys(RegistryAccess access, String structureKey) {
        if (access == null) {
    ...
    public static Set<String> reachableStructureKeys(ServerLevel level, BiomeSource source) {
        if (level == null) {
