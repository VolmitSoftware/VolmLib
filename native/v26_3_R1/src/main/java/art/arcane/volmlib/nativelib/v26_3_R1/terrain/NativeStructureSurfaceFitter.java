...
public final class NativeStructureSurfaceFitter {
    private static final System.Logger LOGGER = System.getLogger(NativeStructureSurfaceFitter.class.getName());
    ...
    public static boolean shouldPrepareSurfaceTerrain(TerrainAdjustment adjustment,
    ...
    public static int surfaceTerrainRadius() {
        return SURFACE_TERRAIN_RADIUS;
    }

    public static int resolveSurfaceTarget(List<SurfaceAnchor> anchors, int worldX, int worldZ,
                                    int originalY) {
    ...
    private static SurfaceResolution resolveSurface(List<SurfaceAnchor> anchors,
        ...
        for (SurfaceAnchor anchor : anchors) {
            int outX = outset(worldX, anchor.minX(), anchor.maxX());
            int outZ = outset(worldZ, anchor.minZ(), anchor.maxZ());
    ...
    private static void warnSourceBudget(
        ...
        if (WARNED_SOURCE_BUDGET.add(structureId)) {
            LOGGER.log(System.Logger.Level.WARNING, "Native structure SOURCE fitting for '"
    ...
    private static VacuumFootprint collectVacuumFootprint(
        ...
        for (NativeStructureTerrainIntegrator.TerrainTarget target : targets) {
            if (target == null || target.terrain() == null
                    || target.terrain().resolvedMode() != StructureTerrainMode.VACUUM
    ...
    private static void warnVacuumBudget(
        ...
        if (WARNED_VACUUM_BUDGET.add(structureId)) {
            LOGGER.log(System.Logger.Level.WARNING, "Native structure VACUUM fitting for '"
                    + structureId + "' exceeded its bounded template budget; "
    ...
    public static SurfaceAnchor surfaceAnchor(BoundingBox bounds, int groundY, int strength) {
        return new SurfaceAnchor(bounds.minX(), bounds.maxX(), bounds.minZ(), bounds.maxZ(),
                groundY - 1, strength);
    }

    public static boolean requiresSurfaceTerrain(StructureStart start) {
        return requiresSurfaceTerrain(new NativeStructureTerrainIntegrator.TerrainTarget(
                null, start, StructureTerrainSettings.SOURCE));
    }

    public static boolean requiresSurfaceTerrain(NativeStructureTerrainIntegrator.TerrainTarget target) {
        if (target == null || target.terrain() == null) {
            return false;
        }
        StructureStart start = target.start();
        ...
        StructureTerrainMode mode = target.terrain().resolvedMode();
        return mode == StructureTerrainMode.VACUUM
    ...
    private static boolean requiresSourceSurfaceTerrain(
        ...
        return target != null && target.terrain() != null
                && target.start() != null && target.start().isValid()
                && target.terrain().resolvedMode() == StructureTerrainMode.SOURCE
    ...
    public static boolean requiresFlattenTerrain(NativeStructureTerrainIntegrator.TerrainTarget target) {
        if (target == null || target.terrain() == null
                || target.terrain().resolvedMode() != StructureTerrainMode.FLATTEN
        ...
        for (NativeStructureTerrainIntegrator.TerrainTarget target : targets) {
            ...
            try {
                ...
                if (WARNED_FLATTEN_BUDGET.add(structureId)) {
                    LOGGER.log(System.Logger.Level.WARNING, "Native structure FLATTEN fitting for '" + structureId
    ...
    public static boolean hasExposedFlattenPiece(
    ...
    public static FlattenResolution resolveFlattenSurface(
        ...
        for (FlattenAnchor anchor : anchors) {
            ...
            int outX = outset(x, source.minX(), source.maxX());
            int outZ = outset(z, source.minZ(), source.maxZ());
            boolean containsColumn = outX == 0 && outZ == 0;
    ...
    public static void applySurfaceColumn(WorldGenLevel world, BlockPos.MutableBlockPos position,
    ...
    public record SurfaceAnchor(int minX, int maxX, int minZ, int maxZ, int meetY, int strength) {
    }

    public record FlattenAnchor(SurfaceAnchor source, int radius, int range) {
    }

    public record FlattenResolution(int targetY, int range) {
    }
