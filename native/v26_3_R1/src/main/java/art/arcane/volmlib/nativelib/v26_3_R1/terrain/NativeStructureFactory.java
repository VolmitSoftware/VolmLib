...
public final class NativeStructureFactory {
    ...
    public static StructureStart generate(GenerationContext context, Holder<Structure> sourceHolder,
                                          StructureStartPlan plan, int references) {
        Structure source = sourceHolder.value();
        Structure configured = configure(
                context.registryAccess(), source, plan.jigsaw(),
                plan.underground(), plan.baseY());
    ...
    public static Structure configure(RegistryAccess registryAccess, Structure source,
                               JigsawSettings configuration,
        ...
        CompoundTag structureTag = NativeJigsawMetadata.encodeJigsaw(registryOps, sourceJigsaw);
    ...
    private static void applyConfiguration(CompoundTag tag, JigsawSettings configuration) {
        if (configuration == null) {
            return;
        }
    ...
    private static void applyDistance(CompoundTag tag, JigsawSettings configuration) {
        Integer horizontal = configuration.getMaxDistanceHorizontal();
        Integer vertical = configuration.getMaxDistanceVertical();
        if (horizontal == null && vertical == null) {
            return;
        }
        int resolvedHorizontal = horizontal == null ? NativeJigsawMetadata.sourceDistance(tag, "horizontal") : horizontal;
        int resolvedVertical = vertical == null ? NativeJigsawMetadata.sourceDistance(tag, "vertical") : vertical;
    ...
    private static void applyHeightmap(CompoundTag tag, JigsawHeightmap heightmap) {
        if (heightmap == null || heightmap.useSource()) {
            return;
        }
        if (heightmap.removeProjection()) {
    ...
    private static void applyDimensionPadding(CompoundTag tag, JigsawSettings configuration) {
    ...
    private static void applyLiquidSettings(CompoundTag tag, JigsawLiquidSettings settings) {
        if (settings == null || settings.useSource()) {
            return;
        }
