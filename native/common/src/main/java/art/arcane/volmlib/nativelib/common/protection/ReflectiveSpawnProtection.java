...
public final class ReflectiveSpawnProtection implements SpawnProtection {
    private final Server server;
    private final Binding binding;
    private final AtomicReference<Throwable> failure = new AtomicReference<>();

    private ReflectiveSpawnProtection(Server server, Binding binding) {
        this.server = server;
        this.binding = binding;
    }

    public static ReflectiveSpawnProtection create(Server server) {
        Server required = Objects.requireNonNull(server, "server");
        try {
            return new ReflectiveSpawnProtection(required, Binding.resolve(required));
        } catch (ReflectiveOperationException | RuntimeException | LinkageError exception) {
            ReflectiveSpawnProtection protection = new ReflectiveSpawnProtection(required, null);
