...
public class ReflectiveEntityVisibilityAccess implements EntityVisibilityAccess {
    ...
    public ReflectiveEntityVisibilityAccess() {
    }

    public boolean isVisible(Player observer, UUID entityId, boolean visibleByDefault, Plugin excludedPlugin) {
    ...
    private static final class VisibilityAccessor {
        ...
        private void reportFailure(Player observer, Exception error) {
            ...
            Logger logger = Bukkit.getServer() == null ? Logger.getLogger("NativeVisibility") : Bukkit.getLogger();
            logger.log(Level.WARNING,
                "Cannot read native player visibility on " + observer.getClass().getName()
                    + "; entities are withheld for this player implementation.",
                error);
        }
    }
}
